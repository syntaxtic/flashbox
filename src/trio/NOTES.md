...
- **Exports:** English (`en`) is the **front** of the card. The **back** is Spanish (`es`) first, then Turkish (`tr`), separated by an em-dash line (`———`) with an empty line before and after — not `---`, which Mochi already uses as a card/side break:
